...
- `AGEWEC2026_提出資料/`を`deliverables/AGEWEC2026/`へ内容変更なしで移動
- 字幕版・BGM版・確定版の動画3件を`deliverables/variants/`へ分類
- 応募フォーム記入案を`docs/submission/`へ移動
- 空になった`other/`を削除し、用途不明な混在領域を解消
- 確定提出物75ファイルの移動前後SHA-256一覧が完全一致
- 派生動画3件と応募フォーム案も移動前後のSHA-256が一致
- 開始時に固定した重要提出物5件のSHA-256が引き続き一致
