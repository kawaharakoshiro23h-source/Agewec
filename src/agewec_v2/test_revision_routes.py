...
使い方:
    ...
    # 1ルートだけ
    uv run python -m agewec_v2.test_revision_routes --route d

    # 実ComfyUIで本当に再生成されるかまで見る（遅い）
    uv run python -m agewec_v2.test_revision_routes \
...
def _config(backend: str, seconds: float) -> dict[str, Any]:
    ...
    # 出力を隔離（本番の runtime/runs を汚さない）
    paths = dict(config.get("paths", {}))
    paths["runtime_dir"] = str(ROOT / "runtime" / "route_test")
...
def main() -> None:
    ...
    print(
        ...
        f"1カット・{args.seconds}秒・draft / 出力: runtime/route_test/"
    )
