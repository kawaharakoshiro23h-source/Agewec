...
def _detected_roots() -> tuple[Path, Path]:
    ...
    if not (project_root / "configs").is_dir() and legacy_workflow.is_dir():
        workflow_root = legacy_workflow
    return workflow_root, project_root
    ...
    def from_config(
        ...
        *,
        ...
        if uses_runtime_layout:
            ...
            legacy_workflow = (
                workflow
                if workflow.name == "workflow_v2"
                else project / "workflow_v2"
            )
            work = _resolve(
                legacy_workflow,
                values.get("work_dir", "work"),
            )
