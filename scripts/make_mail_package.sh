...
#
# 出力:
#   runtime/submissions/agewec_<run_id>_report.html  … 単体で開けるレポート
#   runtime/submissions/agewec_<run_id>_video.mp4    … 視聴用の最終動画
#
# 原本（runtime/submissions/<run_id>/）は変更しない。最終提出には原本を使うこと。
#
# 使い方:
...
SUBMISSIONS_ROOT="$ROOT/runtime/submissions"
SRC="$SUBMISSIONS_ROOT/$RUN_ID"
if [ ! -d "$SRC" ] && [ -d "$ROOT/workflow_v2/submissions/$RUN_ID" ]; then
  SRC="$ROOT/workflow_v2/submissions/$RUN_ID"
fi
WORK="$(mktemp -d)"
trap 'rm -rf "$WORK"' EXIT

mkdir -p "$SUBMISSIONS_ROOT"
OUT_HTML="$SUBMISSIONS_ROOT/agewec_${RUN_ID}_report.html"
OUT_VIDEO="$SUBMISSIONS_ROOT/agewec_${RUN_ID}_video.mp4"
